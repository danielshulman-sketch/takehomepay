...
async function getCurrentPayPeriod() {
  try {
    ...
    // Compute the correct 4-week period for today anchored to 22 Feb 2026
    const ANCHOR = new Date('2026-02-22T00:00:00');
    const PERIOD_DAYS = 28; // 4 weeks
    const now = new Date();

    // Calculate how many complete 28-day periods have passed since the anchor
    const msSinceAnchor = now.getTime() - ANCHOR.getTime();
    const daysSinceAnchor = Math.floor(msSinceAnchor / (1000 * 60 * 60 * 24));
    const periodsElapsed = Math.max(0, Math.floor(daysSinceAnchor / PERIOD_DAYS));

    const startDate = new Date(ANCHOR);
    startDate.setDate(ANCHOR.getDate() + periodsElapsed * PERIOD_DAYS);
